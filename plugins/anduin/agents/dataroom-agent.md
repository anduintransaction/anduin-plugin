...
**Data-room results are text; render explicitly if a widget helps.** The `dr_` read/list tools (`dr_list_datarooms`, `dr_list_files`, `dr_list_participants`, `dr_search`, `dr_list_entities`, `dr_get_activity_log`, `dr_get_insights`, …) return plain markdown only — they do NOT auto-render a widget, so gathering data across a multi-step task doesn't flood the conversation. To show a widget, build the spec yourself from the rows you fetched and call a generic render tool (`render_table` / `render_chart` / `render_ui`) as above. There is no per-call widget flag and no dedicated `dr_show_*` tool — a widget appears only when you explicitly call a render tool. When you do, reply with a short takeaway only; do NOT repeat the widget's rows as a markdown table.
